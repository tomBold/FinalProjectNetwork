...
public:
	...
	void run(){
		...
		for(int j=0;j<TEST_SIZE;j++){
			...
			MultipleTCPSocketsListener m;
			m.addSockets(serverSockets);

			// Use MultipleTCPSocketsListener to find the socket that is ready
			TCPSocket* tcpSocket = m.listenToSocket();
			...
			TCPSocket* peerSocket = tcpSocket->listenAndAccept();
			...
			peerSocket->recv((char*)&msgLen,4);
			msgLen = ntohl(msgLen);
			int rc = peerSocket->recv(buffer, msgLen);
			...
			peerSocket->send((char*)&msgLen,4);
			peerSocket->send(message.data(),message.length());
